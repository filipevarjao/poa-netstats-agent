defmodule POAAgent.Plugins.Transfers.DB.MnesiaTest do
  ...
  test "sending data to mnesia" do
    ...
    assert {:atomic, []} = :mnesia.transaction(read_data)
  ...
  test "sending more data to DB" do
    args = %{name: :metrics_transfer, args: [table_name: :metrics2,
                                             fields: [:timestamp, :os_type,
                                                      :unix_process, :cpu_util,
                                                      :disk_used, :memsup]]}
    ...
    read_data = fn -> :mnesia.match_object({:metrics2, :_, {:unix, :linux}, 101,
                                            10.1, 10, [:ok, 10]}) end
    assert {:atomic, []} = :mnesia.transaction(read_data)
